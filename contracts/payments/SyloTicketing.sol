...
contract SyloTicketing is ISyloTicketing, Initializable, Ownable2StepUpgradeable, ERC165 {
    ...
    function rewardRedeemer(
        ...
        // check sender delegated account
        if (ticket.senderDelegatedAccount != address(0)) {
            if (
                !_authorizedAccount.validatePermission(
                    ticket.sender,
                    ticket.senderDelegatedAccount,
                    permission
                )
            ) {
        ...
        // check receiver delegated account
        if (ticket.receiverDelegatedAccount != address(0)) {
            if (
                !_authorizedAccount.validatePermission(
                    ticket.receiver,
                    ticket.receiverDelegatedAccount,
                    permission
                )
            ) {
    ...
    function requireValidWinningTicket(
        ...
        if (!isValidTicketSig(senderSig, ticket.sender, ticket.delegatedSender, ticketHash)) {
        ...
        if (!isWinningTicket(senderSig, ticket, senderRand, redeemerRand)) {
    ...
    function isWinningTicket(
        ...
        return uint256(keccak256(abi.encodePacked(senderSig, senderRand, redeemerRand))) < prob;
    }
