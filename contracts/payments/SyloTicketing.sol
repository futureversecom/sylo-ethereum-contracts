...
contract SyloTicketing is ISyloTicketing, Initializable, Ownable2StepUpgradeable, ERC165 {
    ...
    error InvalidMerkleProof(address receiver);
    ...
    function redeemMultiReceiver(
        ...
        bytes32 ticketHash = getMultiReceiverTicketHash(ticket, receiver.main);
        requireValidWinningTicket(
            ticketHash,
            ticket.sender,
            receiver,
     ...
     * @param redeemerCommit The redeemer of the ticket.
     * @param redeemerRand The redeemer random value, generated by the Node prior
     * to performing the event relay.
     * @param senderSig The signature of the sender of the ticket.
     * @param receiverSig The signature of the redeemer of the ticket.
    ...
    function requireValidWinningTicket(
        ...
        uint256 epochId,
        uint256 generationBlock,
        bytes32 redeemerCommit,
        uint256 redeemerRand,
        bytes calldata senderSig,
        bytes calldata receiverSig
    ) public view {
        if (sender.main == address(0)) {
        ...
        if (createCommit(generationBlock, redeemerRand) != redeemerCommit) {
            revert RedeemerCommitMismatch();
        }

        if (!hasTicketSigningPermission(sender, generationBlock)) {
            revert InvalidSenderTicketSigningPermission();
        }
        if (!hasTicketSigningPermission(receiver, generationBlock)) {
            revert InvalidReceiverTicketSigningPermission();
        }

        if (!isValidTicketSig(sender, senderSig, ticketHash)) {
        ...
        if (!isWinningTicket(senderSig, receiverSig, epochId, generationBlock, redeemerRand)) {
     ...
     * @param generationBlock The generationBlock of the ticket.
     */
    function calculateWinningProbability(uint256 epochId, uint256 generationBlock) public view returns (uint128) {
    ...
    /**
     * @notice Returns the hash of a multi receiver ticket. Takes all fields in a ticket
     * as inputs to the hash, as well as a specific receiver.
     * @return A byte-array representing the hash.
     */
    function getMultiReceiverTicketHash(MultiReceiverTicket memory ticket, address receiver) public pure returns (bytes32) {
        return
            keccak256(
                abi.encodePacked(
                    ticket.epochId,
