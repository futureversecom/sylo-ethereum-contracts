...
contract('Pricing', accounts => {
  ...
  it('allows a staked account to vote', async () => {
    ...
    await priceManager.vote(1, { from: accounts[1] });
  });

  it('allows updating a vote', async () => {
    await directory.addStake(1, accounts[1], { from: accounts[1] });
    await priceManager.vote(1, { from: accounts[1] });
    await priceManager.vote(2, { from: accounts[1] });
  });
  ...
  it('prevents vote with value set to 0', async () => {
    await directory.addStake(1, accounts[1], { from: accounts[1] });
    await priceManager.vote(0, { from: accounts[1] })
      .then(() => {
  ...
  it('can determince price after votes and staking has occured', async () => {
    for (let i = 0; i < accounts.length; i++) {
      await directory.addStake(10, accounts[i],{ from: accounts[1] });
    }

    const stake = await directory.getTotalStake({ from: accounts[1] });

    const prices = [20, 10, 5, 30, 45, 12, 17, 9, 24, 10];
    

    for (let i = 0; i < accounts.length; i++) {
      await priceManager.vote(prices[i], { from: accounts[i] });
    }
    ...
    await priceManager.calculatePrice(sortedVotes, { from: accounts[1] });

    const currentPrice = await priceManager.currentPrice();
