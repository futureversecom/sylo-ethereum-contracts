...
describe('Staking', () => {
  ...
  it('should be able to set parameters after initialization', async () => {
    await expect(stakingManager.setUnlockDuration(100)).to.emit(stakingManager, 'UnlockDurationUpdated').withArgs(100)
  ...
  it('should be able to stake', async () => {
    ...
    assert.equal(stakeEntry.amount.toString(), '100', 'A stake entry with 100 tokens should be managed by the contract')
  })

  it('should not be able to add more stake if minimum stake proportion not met', async () => {
    await expect(stakingManager.addStake(1, await accounts[1].getAddress())).to.be.revertedWith(
  ...
  it('should fail to calculate remaining stake if owned stake too low', async () => {
    ...
    await expect(stakingManager.calculateMaxAdditionalDelegatedStake(owner)).to.be.revertedWith(
  ...
  it('can not unlock no stake', async () => {
    await expect(stakingManager.unlockStake(100, owner)).to.be.revertedWith('Nothing to unstake')
  })

  it('can not unlock zero stake', async () => {
    await stakingManager.addStake(100, owner)
    await expect(stakingManager.unlockStake(0, owner)).to.be.revertedWith('Cannot unlock with zero amount')
  })

  it('can not unlock more stake than exists', async () => {
    await stakingManager.addStake(100, owner)
    await expect(stakingManager.unlockStake(101, owner)).to.be.revertedWith('Cannot unlock more than staked')
  })
  ...
  it("should not be able to withdraw stake that hasn't unlocked", async () => {
    await stakingManager.addStake(100, owner)
    await stakingManager.unlockStake(100, owner)
    await expect(stakingManager.withdrawStake(owner)).to.be.revertedWith('Stake not yet unlocked')
  })
  ...
  it('should be able to cancel unlocking', async () => {
    ...
    assert.equal(unlocking.amount.toNumber(), 0, 'Expected unlocking to be cancelled')
  })
  ...
  it('should be able to cancel a portion of the unlocking', async () => {
    ...
    assert.equal(unlocking.amount.toNumber(), 46, 'Expected only a portion of the unlocking to be cancelled')
  })
  ...
  it('should not be able to join the next epoch more than once', async () => {
    ...
    await expect(directory.joinNextDirectory(owner)).to.be.revertedWith('Can only join the directory once per epoch')
  })
  ...
  it('should be able to get total stake for a stakee', async () => {
    ...
    for (let i = 2; i < 10; i++) {
      ...
      assert.equal(stakeAmount.toString(), '10', "Expected contract to hold staker's stake")
    }

    const totalStake = await stakingManager.getStakeeTotalManagedStake(owner)

    assert.equal(totalStake.toString(), '180', 'Expected contract to track all stake entries')
  })
  ...
  it('should store the epochId the stake entry was updated at', async () => {
    ...
    assert.equal(stakeEntry.epochId.toNumber(), 1, 'Stake entry should track the epoch id it was updated at')
  })
  ...
  it('should be able to query properties of directory', async () => {
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      assert.equal(stake.toNumber(), 1, 'Expected to be able to query total stake for stakee')
    }

    await updateCurrentDirectory(epochId)

    const totalStake = await directory.getTotalStake(1)
    assert.equal(totalStake.toNumber(), expectedTotalStake, 'Expected to return correct amount for total stake query')

    const entries = await directory.getEntries(1)
    for (let i = 0; i < accounts.length; i++) {
      const address = entries[0][i]
      const boundary = entries[1][i]
      assert.equal(address, await accounts[i].getAddress(), 'Expected entry to hold correct address')
      assert.equal(boundary.toNumber(), i + 1, 'Expected entry to hold correct boundary value')
    }
  })
  ...
  it('cannot scan channel greater than 48', async () => {
    ...
    const pk = crypto.randomBytes(32)
    await expect(directory.scan(pk, epochId, 49)).to.be.revertedWith('Only channels 0-48 are supported')
  })
  ...
  async function addStake(accountNum: number) {
    ...
    await stakingManager.connect(accounts[accountNum]).addStake(1, await accounts[accountNum].getAddress())
